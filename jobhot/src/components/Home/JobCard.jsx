...
const JobCard = ({ job, initialSaved = false, onUnsave }) => {
    ...
    const goToJobOrPromptLogin = () => (isLoggedIn() ? navigate(`/jobs/${job.id}`) : setShowLoginModal(true));
    ...
    const handleSaveClick = async (e) => {
        ...
        try {
            await fetch(`${API}?action=${action}`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ job_id: job.id }) });
            const newSaved = !saved;
            setSaved(newSaved);
            !newSaved && onUnsave?.(job.id);
        } catch (error) {
            console.error('Error saving job:', error);
        }
    };

    const saveIcon = saved ? '❤️' : '🤍';
    const saveColor = saved ? 'text-red-500' : 'text-gray-300 hover:text-red-400';
    const daysClass = `text-xs px-2.5 py-1 rounded-full font-medium ${urgent ? 'bg-red-50 text-red-600' : 'bg-gray-100 text-gray-500'}`;

    const handleCardClick = () => goToJobOrPromptLogin();
    const handleViewClick = (e) => { e.stopPropagation(); goToJobOrPromptLogin(); };
    ...
    return (
        <>
            <div onClick={handleCardClick} className="group h-full cursor-pointer bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-200 overflow-hidden flex flex-col">
                {urgent && <div className="bg-red-500 text-white text-xs font-semibold text-center py-1">⚡ Sắp hết hạn - còn {job.daysLeft} ngày</div>}
                ...
                <div className="p-4 flex flex-col flex-1">
                    <div className="flex justify-between items-start mb-3">
                        <div className="w-12 h-12 rounded-xl bg-purple-50 flex items-center justify-center overflow-hidden border border-purple-100 shrink-0">
                            {job.logo ? <img src={job.logo} alt={job.company} className="w-full h-full object-cover" /> : <span className="text-xl font-bold text-purple-600">{job.company.charAt(0)}</span>}
                        </div>
