...
const TEAM = [
    { name: 'Thái Đức Trí', role: 'Founder', emoji: '🙋' },
    { name: 'Phạm Xuân Chiến', role: 'Founder', emoji: '👩‍💻' },
    { name: 'Nguyễn Bá Thế', role: 'Founder', emoji: '👨‍🎨' },
    { name: 'Đoàn Hà Anh', role: 'Founder', emoji: '👩' },
    { name: 'Nguyễn Ngọc Sơn', role: 'Member', emoji: '👨‍💼' },
];
        ...
        <div className="flex flex-col min-h-screen bg-gray-50">
            ...
            <div className="max-w-5xl mx-auto px-4 py-12 w-full">
                ...
                <div className="flex flex-wrap justify-center gap-4 mb-10">
                    {TEAM.map((m) => (
                        <div key={m.name} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 text-center w-[calc(50%-8px)] sm:w-40 flex flex-col items-center hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                            <div className="w-14 h-14 rounded-full bg-purple-50 flex items-center justify-center text-3xl mb-3 ring-2 ring-purple-100">{m.emoji}</div>
                            <p className="font-semibold text-sm text-gray-800 leading-tight">{m.name}</p>
                            <span className="mt-1.5 inline-block text-xs font-medium px-2.5 py-0.5 rounded-full bg-purple-100 text-purple-700">{m.role}</span>
                        </div>
                    ))}
                </div>
