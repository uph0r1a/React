...
const FEATURES = [
    { icon: '🔍', title: 'Tìm việc thông minh', desc: 'Hệ thống gợi ý việc làm phù hợp theo ngành nghề và kinh nghiệm của bạn.' },
    { icon: '🏢', title: 'Nhà tuyển dụng uy tín', desc: 'Hàng nghìn công ty hàng đầu Việt Nam đang tuyển dụng trên JobHot.' },
    { icon: '📩', title: 'Ứng tuyển nhanh chóng', desc: 'Nộp hồ sơ chỉ với một cú click. Theo dõi trạng thái ứng tuyển trực tiếp.' },
    { icon: '🔒', title: 'Bảo mật thông tin', desc: 'Thông tin cá nhân được mã hoá và bảo vệ theo tiêu chuẩn cao nhất.' },
];
    ...
    <div className="min-h-screen bg-white flex flex-col">
        ...
        <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur border-b border-gray-100 px-6 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
                <Link to="/about" className="text-sm text-gray-600 hover:text-purple-600 transition-colors font-medium hidden sm:block">
                    Về chúng tôi
                </Link>
                <Link to="/contact" className="text-sm text-gray-600 hover:text-purple-600 transition-colors font-medium hidden sm:block">
                    Liên hệ
                </Link>
                <Link to="/login" className="text-sm px-4 py-2 border border-purple-600 text-purple-600 rounded-lg hover:bg-purple-50 transition-colors font-medium">
                    Đăng nhập
                </Link>
                <Link to="/register" className="text-sm px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors font-medium">
                    Đăng ký
                </Link>
            </div>
        </nav>
        ...
        <section className="bg-linear-to-br from-purple-700 via-purple-600 to-purple-500 text-white py-20 px-4 text-center relative overflow-hidden">
            ...
            <div className="relative max-w-3xl mx-auto">
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-center">
                    <Link to="/register" className="px-8 py-3.5 bg-white text-purple-700 rounded-xl font-bold hover:bg-yellow-50 transition-colors shadow-lg text-sm">
                        🚀 Tạo tài khoản miễn phí
                    </Link>
                    <Link to="/login" className="px-8 py-3.5 bg-white/10 text-white border border-white/30 rounded-xl font-semibold hover:bg-white/20 transition-colors text-sm">
        ...
        <section className="bg-white border-b border-gray-100 py-10 px-4">
            <div className="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
                {STATS.map(({ value, label }) => (
                    <div key={label}>
                        <div className="text-3xl font-extrabold text-purple-700">{value}</div>
            ...
            <div className="max-w-5xl mx-auto">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {FEATURES.map(({ icon, title, desc }) => (
                        <div key={title} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex gap-4 items-start hover:shadow-md transition-shadow">
        ...
        <section className="py-14 px-4 bg-white">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {TOP_INDUSTRIES.map(({ icon, name }) => (
                        <Link key={name} to="/register" className="flex flex-col items-center gap-2 p-4 bg-purple-50 hover:bg-purple-100 rounded-2xl transition-colors group">
        ...
        <section className="py-14 px-4 bg-purple-600 text-white">
            <div className="max-w-3xl mx-auto text-center">
                ...
                <Link to="/register" className="inline-block px-8 py-3.5 bg-white text-purple-700 rounded-xl font-bold hover:bg-yellow-50 transition-colors text-sm shadow-lg">
        ...
        <section className="py-14 px-4 bg-gray-50 text-center">
            <div className="max-w-xl mx-auto">
                ...
                <Link to="/register" className="inline-block px-10 py-3.5 bg-purple-600 text-white rounded-xl font-bold hover:bg-purple-700 transition-colors shadow-lg text-sm">
                    Tạo tài khoản ngay 🎉
                </Link>
            </div>
        </section>
